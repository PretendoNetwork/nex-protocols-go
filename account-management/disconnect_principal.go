...
func (protocol *Protocol) DisconnectPrincipal(handler func(err error, client *nex.Client, callID uint32, idPrincipal uint32) uint32) {
...
func (protocol *Protocol) handleDisconnectPrincipal(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.disconnectPrincipalHandler(fmt.Errorf("Failed to read idPrincipal from parameters. %s", err.Error()), client, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.disconnectPrincipalHandler(nil, client, callID, idPrincipal)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
