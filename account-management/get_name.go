...
func (protocol *Protocol) handleGetName(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.GetName(fmt.Errorf("Failed to read idPrincipal from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.GetName(nil, packet, callID, idPrincipal)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
