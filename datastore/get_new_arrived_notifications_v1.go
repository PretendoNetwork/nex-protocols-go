...
func (protocol *Protocol) handleGetNewArrivedNotificationsV1(packet nex.PacketInterface) {
	...
	if err != nil {
		_, rmcError := protocol.GetNewArrivedNotificationsV1(fmt.Errorf("Failed to read param from parameters. %s", err.Error()), packet, callID, param)
		if rmcError != nil {
	...
	rmcMessage, rmcError := protocol.GetNewArrivedNotificationsV1(nil, packet, callID, param)
	if rmcError != nil {
		globals.RespondError(packet, ProtocolID, rmcError)
		return
	}
