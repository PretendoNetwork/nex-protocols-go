...
func (protocol *Protocol) handleGetRating(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.GetRating(fmt.Errorf("Failed to read target from parameters. %s", err.Error()), packet, callID, nil, nil)
		if errorCode != 0 {
	...
	if err != nil {
		_, errorCode = protocol.GetRating(fmt.Errorf("Failed to read accessPassword from parameters. %s", err.Error()), packet, callID, nil, nil)
		if errorCode != 0 {
			globals.RespondError(packet, ProtocolID, errorCode)
		}
