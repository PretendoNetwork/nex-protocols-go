...
func (protocol *Protocol) DeletePokemon(handler func(err error, client *nex.Client, callID uint32, param *datastore_pokemon_gen6_types.GlobalTradeStationDeletePokemonParam) uint32) {
...
func (protocol *Protocol) handleDeletePokemon(packet nex.PacketInterface) {
	...
	errorCode = protocol.deletePokemonHandler(nil, client, callID, param.(*datastore_pokemon_gen6_types.GlobalTradeStationDeletePokemonParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
