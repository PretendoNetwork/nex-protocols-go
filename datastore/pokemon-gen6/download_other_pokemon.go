...
func (protocol *Protocol) DownloadOtherPokemon(handler func(err error, client *nex.Client, callID uint32, param *datastore_pokemon_gen6_types.GlobalTradeStationDownloadOtherPokemonParam)) {
...
func (protocol *Protocol) handleDownloadOtherPokemon(packet nex.PacketInterface) {
	...
	if err != nil {
		go protocol.downloadOtherPokemonHandler(fmt.Errorf("Failed to read param from parameters. %s", err.Error()), client, callID, nil)
		return
	}

	go protocol.downloadOtherPokemonHandler(nil, client, callID, param.(*datastore_pokemon_gen6_types.GlobalTradeStationDownloadOtherPokemonParam))
}
