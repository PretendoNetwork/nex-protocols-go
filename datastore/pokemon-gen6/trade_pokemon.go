...
func (protocol *Protocol) handleTradePokemon(packet nex.PacketInterface) {
	...
	param, err := nex.StreamReadStructure(parametersStream, datastore_pokemon_gen6_types.NewGlobalTradeStationTradePokemonParam())
	if err != nil {
	...
	rmcMessage, errorCode := protocol.TradePokemon(nil, packet, callID, param)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
