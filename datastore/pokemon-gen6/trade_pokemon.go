...
func (protocol *Protocol) handleTradePokemon(packet nex.PacketInterface) {
	if protocol.TradePokemon == nil {
		err := nex.NewError(nex.ResultCodes.Core.NotImplemented, "DataStorePokemonGen6::TradePokemon not implemented")
	...
	param := datastore_pokemon_gen6_types.NewGlobalTradeStationTradePokemonParam()

	err := param.ExtractFrom(parametersStream)
	if err != nil {
		_, rmcError := protocol.TradePokemon(fmt.Errorf("Failed to read param from parameters. %s", err.Error()), packet, callID, nil)
		if rmcError != nil {
