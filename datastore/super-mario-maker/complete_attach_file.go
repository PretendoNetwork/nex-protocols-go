...
func (protocol *Protocol) handleCompleteAttachFile(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.CompleteAttachFile(fmt.Errorf("Failed to read param from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.CompleteAttachFile(nil, packet, callID, param.(*datastore_types.DataStoreCompletePostParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
