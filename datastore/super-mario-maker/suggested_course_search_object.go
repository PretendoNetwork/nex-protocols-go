...
func (protocol *Protocol) handleSuggestedCourseSearchObject(packet nex.PacketInterface) {
	if protocol.SuggestedCourseSearchObject == nil {
		err := nex.NewError(nex.ResultCodes.Core.NotImplemented, "DataStoreSuperMarioMaker::SuggestedCourseSearchObject not implemented")
	...
	rmcMessage, rmcError := protocol.SuggestedCourseSearchObject(nil, packet, callID, param, extraData)
	if rmcError != nil {
		globals.RespondError(packet, ProtocolID, rmcError)
		return
	}
