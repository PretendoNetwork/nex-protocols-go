...
func (protocol *Protocol) handlePostFightingPowerScore(packet nex.PacketInterface) {
	...
	var params types.List[datastore_super_smash_bros_4_types.DataStorePostFightingPowerScoreParam]

	err := params.ExtractFrom(parametersStream)
	if err != nil {
		_, rmcError := protocol.PostFightingPowerScore(fmt.Errorf("Failed to read params from parameters. %s", err.Error()), packet, callID, params)
		if rmcError != nil {
