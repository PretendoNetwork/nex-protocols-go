...
func (protocol *Protocol) handleSearchSharedData(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.SearchSharedData(fmt.Errorf("Failed to read param from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	errorCode = protocol.SearchSharedData(nil, packet, callID, param.(*datastore_super_smash_bros_4_types.DataStoreSearchSharedDataParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
