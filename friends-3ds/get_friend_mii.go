...
func (protocol *Protocol) handleGetFriendMii(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.GetFriendMii(fmt.Errorf("Failed to read pidList from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	errorCode = protocol.GetFriendMii(nil, packet, callID, pidList)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
