...
func (protocol *Protocol) handleCancelFriendRequest(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.CancelFriendRequest(fmt.Errorf("Failed to read id from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.CancelFriendRequest(nil, packet, callID, id)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
