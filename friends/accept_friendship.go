...
func (protocol *Protocol) handleAcceptFriendship(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.AcceptFriendship(fmt.Errorf("Failed to read uiPlayer from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.AcceptFriendship(nil, packet, callID, uiPlayer)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
