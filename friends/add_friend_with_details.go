...
func (protocol *Protocol) handleAddFriendByNameWithDetails(packet nex.PacketInterface) {
	...
	if err != nil {
		_, rmcError := protocol.AddFriendByNameWithDetails(fmt.Errorf("Failed to read uiPlayer from parameters. %s", err.Error()), packet, callID, uiPlayer, uiDetails, strMessage)
		if rmcError != nil {
	...
	if err != nil {
		_, rmcError := protocol.AddFriendByNameWithDetails(fmt.Errorf("Failed to read uiDetails from parameters. %s", err.Error()), packet, callID, uiPlayer, uiDetails, strMessage)
		if rmcError != nil {
			globals.RespondError(packet, ProtocolID, rmcError)
		}
	...
	if err != nil {
		_, rmcError := protocol.AddFriendByNameWithDetails(fmt.Errorf("Failed to read strMessage from parameters. %s", err.Error()), packet, callID, uiPlayer, uiDetails, strMessage)
		if rmcError != nil {
			globals.RespondError(packet, ProtocolID, rmcError)
		}
