...
func (protocol *Protocol) handleCancelInvitation(packet nex.PacketInterface) {
	...
	if err != nil {
		_, rmcError := protocol.CancelInvitation(fmt.Errorf("Failed to read idGathering from parameters. %s", err.Error()), packet, callID, nil, nil, nil)
		if rmcError != nil {
	...
	if err != nil {
		_, rmcError := protocol.CancelInvitation(fmt.Errorf("Failed to read lstPrincipals from parameters. %s", err.Error()), packet, callID, nil, nil, nil)
		if rmcError != nil {
			globals.RespondError(packet, ProtocolID, rmcError)
		}
	...
	if err != nil {
		_, rmcError := protocol.CancelInvitation(fmt.Errorf("Failed to read strMessage from parameters. %s", err.Error()), packet, callID, nil, nil, nil)
		if rmcError != nil {
			globals.RespondError(packet, ProtocolID, rmcError)
		}
