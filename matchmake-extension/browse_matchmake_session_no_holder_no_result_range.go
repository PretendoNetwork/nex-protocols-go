...
func (protocol *Protocol) BrowseMatchmakeSessionNoHolderNoResultRange(handler func(err error, client *nex.Client, callID uint32, searchCriteria *match_making_types.MatchmakeSessionSearchCriteria) uint32) {
...
func (protocol *Protocol) handleBrowseMatchmakeSessionNoHolderNoResultRange(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.browseMatchmakeSessionNoHolderNoResultRangeHandler(fmt.Errorf("Failed to read searchCriteria from parameters. %s", err.Error()), client, callID, nil)
		if errorCode != 0 {
	...
	errorCode = protocol.browseMatchmakeSessionNoHolderNoResultRangeHandler(nil, client, callID, searchCriteria.(*match_making_types.MatchmakeSessionSearchCriteria))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
