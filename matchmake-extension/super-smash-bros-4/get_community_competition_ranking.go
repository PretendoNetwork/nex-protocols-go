...
func (protocol *Protocol) handleGetCommunityCompetitionRanking(packet nex.PacketInterface) {
	if protocol.GetCommunityCompetitionRanking == nil {
		err := nex.NewError(nex.ResultCodes.Core.NotImplemented, "MatchmakeExtensionSuperSmashBros4::GetCommunityCompetitionRanking not implemented")
	...
	rmcMessage, rmcError := protocol.GetCommunityCompetitionRanking(nil, packet, callID, packet.Payload())
	if rmcError != nil {
