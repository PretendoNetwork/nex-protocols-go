...
func (protocol *Protocol) handleUnregisterCommunityCompetition(packet nex.PacketInterface) {
	if protocol.UnregisterCommunityCompetition == nil {
		err := nex.NewError(nex.ResultCodes.Core.NotImplemented, "MatchmakeExtensionSuperSmashBros4::UnregisterCommunityCompetition not implemented")
	...
	rmcMessage, rmcError := protocol.UnregisterCommunityCompetition(nil, packet, callID, packet.Payload())
	if rmcError != nil {
