...
func (protocol *Protocol) handleGetStatsAll(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.getStatsAllHandler(fmt.Errorf("Failed to read target from parameters. %s", err.Error()), client, callID, nil)
		if errorCode != 0 {
	...
	errorCode = protocol.getStatsAllHandler(nil, client, callID, target.(*matchmake_referee_types.MatchmakeRefereeStatsTarget))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
