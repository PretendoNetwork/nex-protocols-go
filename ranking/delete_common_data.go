...
func (protocol *Protocol) handleDeleteCommonData(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.DeleteCommonData(fmt.Errorf("Failed to read uniqueID from parameters. %s", err.Error()), packet, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.DeleteCommonData(nil, packet, callID, uniqueID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
