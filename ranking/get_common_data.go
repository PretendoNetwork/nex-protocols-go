...
func (protocol *Protocol) handleGetCommonData(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.GetCommonData(fmt.Errorf("Failed to read uniqueID from parameters. %s", err.Error()), packet, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.GetCommonData(nil, packet, callID, uniqueID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
