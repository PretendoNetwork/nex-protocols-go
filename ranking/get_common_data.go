...
func (protocol *Protocol) handleGetCommonData(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.GetCommonData(fmt.Errorf("Failed to read uniqueID from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.GetCommonData(nil, packet, callID, uniqueID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
