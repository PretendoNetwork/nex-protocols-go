...
func (protocol *Protocol) handleGetBalanceRequest(packet nex.PacketInterface) {
	...
	rmcMessage, errorCode := protocol.GetBalanceRequest(nil, packet, callID, getBalanceParam.(*service_item_team_kirby_clash_deluxe_types.ServiceItemGetBalanceParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
