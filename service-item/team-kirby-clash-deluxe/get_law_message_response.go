...
func (protocol *Protocol) handleGetLawMessageResponse(packet nex.PacketInterface) {
	...
	if err != nil {
		_, errorCode = protocol.GetLawMessageResponse(fmt.Errorf("Failed to read requestID from parameters. %s", err.Error()), packet, callID, nil)
		if errorCode != 0 {
	...
	rmcMessage, errorCode := protocol.GetLawMessageResponse(nil, packet, callID, requestID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
		return
	}
