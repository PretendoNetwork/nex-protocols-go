...
func (protocol *Protocol) GetServiceItemRightResponse(handler func(err error, client *nex.Client, callID uint32, requestID uint32) uint32) {
...
func (protocol *Protocol) handleGetServiceItemRightResponse(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.getServiceItemRightResponseHandler(fmt.Errorf("Failed to read requestID from parameters. %s", err.Error()), client, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.getServiceItemRightResponseHandler(nil, client, callID, requestID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
