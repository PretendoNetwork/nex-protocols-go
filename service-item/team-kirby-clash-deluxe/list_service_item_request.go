...
func (protocol *Protocol) handleListServiceItemRequest(packet nex.PacketInterface) {
	...
	errorCode = protocol.ListServiceItemRequest(nil, packet, callID, listServiceItemParam.(*service_item_team_kirby_clash_deluxe_types.ServiceItemListServiceItemParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
