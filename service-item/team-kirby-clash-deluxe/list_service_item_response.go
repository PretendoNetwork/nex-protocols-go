...
func (protocol *Protocol) handleListServiceItemResponse(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.listServiceItemResponseHandler(fmt.Errorf("Failed to read requestID from parameters. %s", err.Error()), client, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.listServiceItemResponseHandler(nil, client, callID, requestID)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
