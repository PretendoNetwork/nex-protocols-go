...
func (protocol *Protocol) handlePurchaseServiceItemResponse(packet nex.PacketInterface) {
	if protocol.PurchaseServiceItemResponse == nil {
		err := nex.NewError(nex.ResultCodes.Core.NotImplemented, "ServiceItemTeamKirbyClashDeluxe::PurchaseServiceItemResponse not implemented")
	...
	if err != nil {
		_, rmcError := protocol.PurchaseServiceItemResponse(fmt.Errorf("Failed to read requestID from parameters. %s", err.Error()), packet, callID, requestID)
		if rmcError != nil {
