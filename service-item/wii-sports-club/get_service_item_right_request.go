...
func (protocol *Protocol) handleGetServiceItemRightRequest(packet nex.PacketInterface) {
	...
	if err != nil {
		_, rmcError := protocol.GetServiceItemRightRequest(fmt.Errorf("Failed to read getServiceItemRightParam from parameters. %s", err.Error()), packet, callID, getServiceItemRightParam)
		if rmcError != nil {
