...
func (protocol *Protocol) handlePurchaseServiceItemRequest(packet nex.PacketInterface) {
	...
	errorCode = protocol.PurchaseServiceItemRequest(nil, packet, callID, purchaseServiceItemParam.(*service_item_wii_sports_club_types.ServiceItemPurchaseServiceItemParam))
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
