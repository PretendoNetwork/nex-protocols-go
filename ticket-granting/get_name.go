...
func (protocol *Protocol) handleGetName(packet nex.PacketInterface) {
	...
	if err != nil {
		errorCode = protocol.getNameHandler(fmt.Errorf("Failed to read id from parameters. %s", err.Error()), client, callID, 0)
		if errorCode != 0 {
	...
	errorCode = protocol.getNameHandler(nil, client, callID, id)
	if errorCode != 0 {
		globals.RespondError(packet, ProtocolID, errorCode)
	}
}
